...
<template>
  <Listbox v-model="selected" as="div" @update:model-value="(value) => $emit('selected', value)">
    <div class="relative mt-2">
      <ListboxButton
          class="relative w-full cursor-default rounded-md bg-white py-1.5 pl-3 pr-10 text-left text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-600 sm:text-sm sm:leading-6">
        <span class="block truncate p-2">{{ selected ? selected['name'] : label }}</span>
      ...
      <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100"
        ...
        <ListboxOptions
          ...
          <ListboxOption v-for="item in items" :key="item" v-slot="{ active, selected }" :value="item"
                         as="template">
