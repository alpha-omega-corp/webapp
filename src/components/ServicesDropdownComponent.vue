...
import {GetServicesResponse, Role, Service} from "@/models/permissions";
...
<template>
  <Listbox v-model="selectedService" as="div" @update:model-value="(value: Role) => $emit('selected', value)">
    ...
    <div class="relative mt-2">
      ...
      <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100"
        ...
        <ListboxOptions
          ...
          <ListboxOption v-for="service in services" :key="service.id" v-slot="{ active, selected }" :value="service"
                         as="template">
            <li :class="[active ? 'bg-indigo-600 text-white' : 'text-gray-900', 'relative cursor-default select-none py-2 pl-8 pr-4']">
              <span :class="[selected ? 'font-semibold' : 'font-normal', 'block truncate']">{{ service.name }}</span>
