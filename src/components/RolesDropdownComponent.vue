...
import {GetRolesResponse, Role} from "@assets/models/permissions";
...
<template>
  <Listbox as="div" v-model="selectedRole" @update:model-value="(value: Role) => $emit('selected', value)">
    ...
    <div class="relative mt-2">
      ...
      <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100" leave-to-class="opacity-0">
        <ListboxOptions class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
          <ListboxOption as="template" v-for="role in roles" :key="role.name" :value="role" v-slot="{ active, selected }">
